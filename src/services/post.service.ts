...
function mapPostToFull<T extends { tags: { tag: Tag }[] }>(p: T) {
  const { tags, ...rest } = p;
  return {
    ...rest,
    tags: normalizeTags(p.tags),
...
export class PostService {
  ...
  async getAllByTag(tag: string) {
    ...
    return posts.map((p) => mapPostToPreview(p));
  }
  ...
  async delete(id: string, userId: string) {
    ...
    if (!post)
      throw ApiError.NotFoundError(
        "К сожалению, мы не смогли найти такую статью",
      );
  ...
  async update(data: UpdatePostRequestDto, postId: string, userId: string) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      return tx.post.findUnique({
        where: { id: postId },
        include: { author: includeAuthor, tags: includeTags },
      });
    });
  }
  async likeOrDislike(
    userId: string,
    postId: string,
    value: typeof VOTE_LIKE | typeof VOTE_DISLIKE,
  ) {
    return prisma.$transaction(async (tx) => {
