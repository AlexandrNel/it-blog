...
function getUserId(req: Request): string {
  ...
  if (!id) throw ApiError.NotFoundError("Не передан id пользователя");
  return id;
}
...
export const getAllByUserId = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const post = await postService.getAllByUserId(id);
    res.json(post as PostPreviewResponseDto);
  },
);
...
export const getSomeByTag = asyncHandler(
  async (req: Request, res: Response) => {
...
export const likeOrDislikePost = asyncHandler(
  async (req: Request, res: Response) => {
...
export const incrementView = asyncHandler(
  async (req: Request, res: Response) => {
...
export const checkIsAuthor = asyncHandler(
  async (req: Request, res: Response) => {
    const user = getUser(req);
    const postId = getParamId(req);
