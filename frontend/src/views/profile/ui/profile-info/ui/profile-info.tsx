...
const renderMap = {
  email: (value: string) => <PublicLink key={value} label={value} link={`mailto:${value}`} icon={<Mail size={16} />} />,

  site: (value: string) => <PublicLink key={value} link={value} label={value} icon={<Globe size={16} />} />,

  github: (value: string) => <SocialLink key={value} label="GitHub" link={value} icon={<GitHubIcon />} />,

  telegram: (value: string) => <SocialLink key={value} label="Telegram" link={value} icon={<Send size={16} />} />,
...
function ProfileInfoView({ data }: { data: ProfileItem[] }) {
  return (
    <Card>
      <CardContent>
        <h2 className="text-base font-bold mb-1">О профиле</h2>
        {data.length ? (
          <Column gap={"sm"}>
            <p className="text-muted-foreground text-sm leading-tight ">Краткая информация и публичные ссылки автора</p>
            {data.map((item) => renderMap[item.type](item.value))}
          </Column>
        ) : (
          <p className=" text-sm">Профиль не настроен</p>
        )}
      </CardContent>
    </Card>
  );
...
function PublicLink({ link, icon, label }: { link: string; label: string; icon?: ReactNode }) {
  return (
    <Link target="_blank" href={link as Route} className="flex items-center gap-2">
      {icon}
      {label}
    </Link>
  );
